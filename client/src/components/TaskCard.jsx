...
export default function TaskCard({ task, onDelete }) {
  ...
  const time = new Date(task.createdAt || task.scheduledAt).toLocaleTimeString(
    "en-US",
    {
      hour: "2-digit",
      minute: "2-digit",
  ...
  return (
    <div style={styles.card}>
      ...
      <div style={styles.meta}>
        <span>
          Priority: <b style={{ color: "#fff" }}>{task.priority}</b>
        </span>
        <span>
          Time: <b style={{ color: "#aaa" }}>{time}</b>
        </span>
        {task.workerId && (
          <span>
            Worker:{" "}
            <b style={{ color: "#00aaff", fontSize: "0.65rem" }}>
              {task.workerId}
            </b>
          </span>
        )}
      ...
      <div style={styles.actions}>
        {task.status === "pending" && (
          <button
            onClick={() => dispatch(cancelTask(task._id))}
