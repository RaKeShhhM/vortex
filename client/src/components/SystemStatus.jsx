  ...
  return (
    <div style={styles.container}>
      ...
      <div style={styles.grid}>
        <StatBox
          label="Queue Size"
          value={scheduler?.queueSize ?? 0}
          color="#00ff88"
        />
        <StatBox
          label="Active Workers"
          value={scheduler?.activeWorkers ?? 0}
          color="#00aaff"
        />
        <StatBox
          label="RAM Used"
          value={`${memUsed}%`}
          color={memUsed > 80 ? "#ff4444" : "#ffaa00"}
        />
        <StatBox
          label="CPU Load"
          value={`${cpuLoad}%`}
          color={cpuLoad > 80 ? "#ff4444" : "#ffaa00"}
        />
      </div>
      {scheduler?.nextTask && (
        <div style={styles.next}>
          Next:{" "}
          <span style={{ color: "#00ff88" }}>{scheduler.nextTask.name}</span> P
          {scheduler.nextTask.priority}
        </div>
      )}
      <div style={styles.uptime}>
        Uptime: {system?.uptime} · {system?.hostname}
      </div>
    </div>
  );
}
