...
export const deleteTasksByDay = createAsyncThunk(
  "tasks/deleteByDay",
  async (date, { rejectWithValue }) => {
    try {
      const res = await api.delete("/tasks/bulk/day", { data: { date } });
...
export const fetchSchedulerStatus = createAsyncThunk(
  ...
  async (_, { rejectWithValue }) => {
    try {
      const res = await api.get("/tasks/scheduler/status");
      return res.data;
    } catch (err) {
      return rejectWithValue(err.response?.data?.message);
    }
...
const taskSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(fetchSchedulerStatus.fulfilled, (state, action) => {
        state.schedulerStatus = action.payload.data;
      });
  },
});
