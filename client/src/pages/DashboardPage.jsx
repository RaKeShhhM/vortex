...
export default function DashboardPage() {
  ...
  const handleDeleteDay = async (dateStr) => {
    if (!window.confirm(`Delete all tasks from ${formatDayLabel(dateStr)}?`))
      return;
      ...
      <div style={styles.content}>
        ...
        <div style={styles.header}>
          <div>
            ...
            <p style={styles.pageSub}>
              {list.length} total · Welcome,{" "}
              <span style={{ color: "#00ff88" }}>{user?.name}</span>
            </p>
          </div>
          <button onClick={() => setShowModal(true)} style={styles.newTaskBtn}>
            + New Task
          </button>
        </div>
        ...
        {!loading && list.length === 0 && (
          <div style={styles.empty}>
            No tasks yet.{" "}
            <span onClick={() => setShowModal(true)} style={styles.emptyLink}>
              Create one →
            </span>
          </div>
        )}
        ...
        {days.map((dateStr) => {
          ...
          const hours = Object.keys(hourGroups).sort((a, b) =>
            b.localeCompare(a),
          ); // latest hour first
          const totalTasksInDay = hours.reduce(
            (acc, h) => acc + hourGroups[h].length,
            0,
          );
