      ...
      [
        {
          ...
          user: req.user._id,
          event: "scheduled",
          message: `Task "${name}" scheduled by ${req.user.name}`,
        },
      ],
      { session },
    );
...
const cancelTask = async (req, res) => {
  try {
    ...
    if (task.status === "running")
      return res
        .status(400)
        .json({ success: false, message: "Cannot cancel a running task" });
  ...
  try {
    res.json({
      ...
      data: {
        ...
        system: resourceMonitor.getSystemSnapshot(),
      },
    });
  } catch (error) {
    res.status(500).json({ success: false, message: error.message });
...
const deleteTask = async (req, res) => {
  try {
    ...
    if (task.status === "running")
      return res
        .status(400)
        .json({ success: false, message: "Cannot delete a running task" });
